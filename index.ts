...
let SyntaxName: { [kind: number]: string } = (<any>ts).SyntaxKind;
...
[
    ...
    const kind = (<any>ts).SyntaxKind[aliasName];
...
function convertPosition(sourceFile: ts.SourceFile, pos: number): ESTree.Position {
    let { line, character: column } = sourceFile.getLineAndCharacterOfPosition(pos);
    line++; // TypeScript uses 0-based lines while ESTree uses 1-based
    return { line, column };
}

function unexpected(node: ts.Node) {
    let { line, column } = convertPosition(node.getSourceFile(), node.pos);
...
function convertClassLikeDeclaration(node: ts.ClassLikeDeclaration, asExpression?: boolean) {
    let superClass: ESTree.Expression = null;
...
function convertArrayBindingElement(node: ts.BindingElement) {
    ...
    const name = convertDeclarationName(node.name);
                ...
                properties: (<ts.ObjectLiteralExpression>node).properties.map(node => {
                    switch (node.kind) {
                        ...
                        case ts.SyntaxKind.PropertyAssignment: {
                            const isShorthand: boolean = node.kind === ts.SyntaxKind.ShorthandPropertyAssignment;
...
function convertTemplateExpression(node: ts.TemplateExpression) {
    const spansCount = node.templateSpans.length;
    let quasis = [convertTemplateSpanLiteral(node.head, true, spansCount === 0)];
...
function convertImportClause(node: ts.ImportClause) {
    let specifiers: Array<ESTree.ImportSpecifier | ESTree.ImportDefaultSpecifier | ESTree.ImportNamespaceSpecifier> = [];
    if (node == null) return specifiers;
    let { name, namedBindings } = node;
    if (name) {
...
function convertVariableStatement(node: ts.VariableStatement) {
    const { flags, declarations } = node.declarationList;
...
function convertPrefixOrPostfixUnaryExpression(node: ts_PrefixOrPostfixUnaryExpression) {
    let operator = tryConvertUpdateOperator(node.operator);
    const isUnary: boolean = operator === undefined;
    if (isUnary)
...
function convertArrowFunction(node: ts.ArrowFunction) {
    let arrowFn = <ESTree.ArrowFunctionExpression>convertFunction(node, 'ArrowFunctionExpression', IdBehavior.Ignore, true);
...
function convertObjectBindingElement(node: ts.BindingElement) {
    const isShorthand: boolean = !node.propertyName;
    let value = convertIdentifierOrBindingPattern(node.name);
    if (node.initializer) {
